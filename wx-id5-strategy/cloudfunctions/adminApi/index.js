 ...
 *   announcementList      -> 返回公告列表
 *   announcementCreate    -> 新建公告
 *   announcementUpdate    -> 编辑公告标题/内容
 *   announcementToggle    -> 公告上线/下线
...
// 公告集合自愈：首次使用时自动创建（集合不存在时 count 会抛错）
let announcementsEnsured = false;
async function ensureAnnouncementsCollection() {
  if (announcementsEnsured) return;
  try {
    await db.collection('announcements').count();
    announcementsEnsured = true;
  } catch (e) {
    try {
      await db.createCollection('announcements');
      announcementsEnsured = true;
    } catch (e2) { console.warn('[adminApi] 创建 announcements 集合失败', e2 && e2.message); }
  }
}
...
exports.main = async (event) => {
  ...
  try {
    ...
    // ===== 公告管理（announcements 集合）=====

    if (action === 'announcementList' || action === 'announcementCreate') {
      await ensureAnnouncementsCollection();
    }

    if (action === 'announcementList') {
      const res = await db.collection('announcements').orderBy('createTime', 'desc').limit(50).get();
      return { code: 0, data: { list: res.data || [] }, message: 'success' };
    }

    if (action === 'announcementCreate') {
      const title = String(event && event.title || '').trim();
      const content = String(event && event.content || '').trim();
      if (!title || !content) return { code: -1, data: null, message: '标题和内容不能为空' };
      if (title.length > 30) return { code: -1, data: null, message: '标题不能超过 30 字' };
      if (content.length > 500) return { code: -1, data: null, message: '内容不能超过 500 字' };
      // 单活跃公告：新公告上线时自动下线其他公告，避免首页横幅堆叠
      if (event.active !== false) {
        await db.collection('announcements').where({ active: true }).update({ data: { active: false, updateTime: db.serverDate() } });
      }
    ...
    if (action === 'announcementUpdate') {
      const id = event.id;
      const title = String(event && event.title || '').trim();
      const content = String(event && event.content || '').trim();
      if (!id || !title || !content) return { code: -1, data: null, message: '参数错误' };
      if (title.length > 30) return { code: -1, data: null, message: '标题不能超过 30 字' };
      if (content.length > 500) return { code: -1, data: null, message: '内容不能超过 500 字' };
      await db.collection('announcements').doc(id).update({
        data: { title: title, content: content, updateTime: db.serverDate() }
      });
      return { code: 0, data: null, message: 'success' };
    }

    if (action === 'announcementToggle') {
      const id = event.id;
      const active = Boolean(event.active);
      if (!id) return { code: -1, data: null, message: '参数错误' };
      if (active) {
        await db.collection('announcements').where({ active: true }).update({ data: { active: false, updateTime: db.serverDate() } });
      }
      await db.collection('announcements').doc(id).update({
        data: { active: active, updateTime: db.serverDate() }
      });
      return { code: 0, data: null, message: 'success' };
    }

    if (action === 'announcementDelete') {
      const id = event.id;
      if (!id) return { code: -1, data: null, message: '参数错误' };
