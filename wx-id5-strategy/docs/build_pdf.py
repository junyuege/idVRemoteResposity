...
cover_rows = [
    ...
    ['质量基线', 'node tools/validate-project.js 通过 1285 项检查'],
]
...
story.append(TBL(
    ...
    [
        ...
        ['工程工具', 'Node.js + PowerShell 脚本', '素材同步压缩、云映射生成、1285 项完整性校验'],
    ],
...
story.append(TBL(
    ...
    [
        ...
        ['完整性校验', '1285 项', 'node tools/validate-project.js 全部通过'],
    ],
...
story.append(TBL(
    ...
    [
        ...
        ['tools/validate-project.js', '项目体检脚本：注册、绑定、WXML 结构、数据流、页面模拟共 1285 项检查。'],
...
story.append(P('tools/validate-project.js 不是简单 lint，而是一个“可执行的发布前验收”：它会注册每个页面、模拟 wx API、跑首页/查询页/详情页/图鉴页流程、检查每条索引图片是否存在本地兜底文件、检查 cloud:// 映射、检查旧 id 解析、检查作者切换是否串数据。目前 1285 项检查全部通过。'))
...
story.append(TBL(
    ...
    [
        ...
        ['工具链', 'tools/*', 'validate-project.js 做 1285 项发布前检查；gen-cloud-assets.js 生成云映射；sync-assets.js 跨平台压缩同步素材；smoke-test.js 全路由冒烟测试。'],
    ],
...
story.append(TBL(
    ...
    [
        ['0. 环境准备（约10分钟）', '能打开并校验项目', '用微信开发者工具打开项目；终端运行 node tools/validate-project.js；通读 PROJECT_PLAN.md。', 'project.config.json、app.json、PROJECT_PLAN.md、tools/validate-project.js', '校验输出 Validation passed: 1285 checks；知道主包 11 页、7 个分包。'],
        ...
        ['5. 读维护工具（约40分钟）', '能安全新增内容', '通读 validate-project.js 的检查分类；按 CLOUD_STORAGE_GUIDE.md 新增作者流程走一遍（可只做 DryRun）。', 'tools/validate-project.js、tools/sync-assets.js、tools/gen-cloud-assets.js、tools/smoke-test.js', '能按 7 步新增一条路线而不破坏 1285 项检查；理解“先传云、再生成映射”。'],
    ],
...
story.append(TBL(
    ['环节', '检查项', '通过标准'],
    [
        ['代码与索引', '运行 node tools/validate-project.js', 'Validation passed: 1285 checks，零失败'],
...
# ============ 9 本轮改造记录 ============
story += H1('9. 本轮改造记录')
story.append(P('以下改造已完成并通过 node tools/validate-project.js 全部校验（1285 checks）。'))
story.append(TBL(
    ['改造项', '改动内容', '效果'],
    [
        ...
        ['校验工具增强', '覆盖 bindlongpress 与分包页面；新增图片扩展名/文件头一致性检查；新增云配置与图标回退检查', '校验项从 773 提升到 1285，堵住原有盲区'],
        ...
        ['图片高度回退', '固定高度影响查看，详情页已恢复 widthFix 自然高度', '保证攻略图完整可读'],
    ],
