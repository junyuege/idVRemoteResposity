...
cover_rows = [
    ...
    ['质量基线', 'node tools/validate-project.js 通过 1286 项检查'],
]
...
story.append(TBL(
    ...
    [
        ...
        ['工程工具', 'Node.js + PowerShell 脚本', '素材同步压缩、云映射生成、1286 项完整性校验'],
    ],
...
story.append(TBL(
    ...
    [
        ...
        ['完整性校验', '1286 项', 'node tools/validate-project.js 全部通过'],
    ],
...
story.append(TBL(
    ...
    [
        ...
        ['tools/validate-project.js', '项目体检脚本：注册、绑定、WXML 结构、数据流、页面模拟共 1286 项检查。'],
...
story.append(P('tools/validate-project.js 不是简单 lint，而是一个“可执行的发布前验收”：它会注册每个页面、模拟 wx API、跑首页/查询页/详情页/图鉴页流程、检查每条索引图片是否存在本地兜底文件、检查 cloud:// 映射、检查旧 id 解析、检查作者切换是否串数据。目前 1286 项检查全部通过。'))
...
story.append(TBL(
    ...
    [
        ...
        ['资料图鉴', 'pages/inventory', '异象/道具/辞章三类 Tab；关键词搜索；异象刷新难度筛选；道具/辞章品质筛选；品质映射（独特蓝/奇珍紫/稀世金/华彩红）；强化异象红框；点击底部抽屉展示刷新地图、应对方式、价格、重量、耐久等详情；图片失败回退占位图。'],
        ['小抄教学', 'pages/tutorial', '展示教学图片；加载失败重试；点击全屏预览；支持好友与朋友圈分享。'],
        ...
        ['工具链', 'tools/*', 'validate-project.js 做 1286 项发布前检查；gen-cloud-assets.js 生成云映射；sync-assets.js 跨平台压缩同步素材；smoke-test.js 全路由冒烟测试。'],
    ],
...
story.append(TBL(
    ...
    [
        ['0. 环境准备（约10分钟）', '能打开并校验项目', '用微信开发者工具打开项目；终端运行 node tools/validate-project.js；通读 PROJECT_PLAN.md。', 'project.config.json、app.json、PROJECT_PLAN.md、tools/validate-project.js', '校验输出 Validation passed: 1286 checks；知道主包 11 页、7 个分包。'],
        ...
        ['5. 读维护工具（约40分钟）', '能安全新增内容', '通读 validate-project.js 的检查分类；按 CLOUD_STORAGE_GUIDE.md 新增作者流程走一遍（可只做 DryRun）。', 'tools/validate-project.js、tools/sync-assets.js、tools/gen-cloud-assets.js、tools/smoke-test.js', '能按 7 步新增一条路线而不破坏 1286 项检查；理解“先传云、再生成映射”。'],
    ],
...
story.append(TBL(
    ['环节', '检查项', '通过标准'],
    [
        ['代码与索引', '运行 node tools/validate-project.js', 'Validation passed: 1286 checks，零失败'],
...
# ============ 9 本轮改造记录 ============
story += H1('9. 本轮改造记录')
story.append(P('以下改造已完成并通过 node tools/validate-project.js 全部校验（1286 checks）。'))
story.append(TBL(
    ['改造项', '改动内容', '效果'],
    [
        ...
        ['校验工具增强', '覆盖 bindlongpress 与分包页面；新增图片扩展名/文件头一致性检查；新增云配置与图标回退检查', '校验项从 773 提升到 1286，堵住原有盲区'],
        ...
        ['CI 自动校验', '新增 .github/workflows/validate.yml，每次提交运行 validate-project 与 smoke-test', '防止回归进入主分支'],
        ['图鉴筛选', '异象保持介绍式浏览；道具/辞章支持品质筛选', '避免过度筛选，突出介绍'],
        ['教学与分享增强', '教学图增加失败重试；首页/查询页/详情页/教学页支持朋友圈分享', '异常状态更完整，传播入口更多'],
        ['最近查看历史', '最多保存 10 条；首页改为浮动“历史”按钮 + 底部半屏面板，默认显示 2 条并支持展开/清空', '完全不挤压地图攻略入口'],
        ['数据文件兼容修复', '拆分文件在微信开发者工具出现嵌套 require 解析异常，已合并回 data/localMapIndex.js 单文件', '确保开发者工具可正常编译'],
        ['匿名统计', '新增 addAnalytics 云函数与 utils/analytics.js；上报页面访问、图片失败、无结果搜索', '为内容维护优先级提供数据'],
        ['图标秒开优化', '选中凉哈皮路线时同时预下载 pkg-lianghapi-icons 并预取 28 张图标临时链接，openShape 直接复用缓存', '图标弹层接近秒开'],
        ['路线图临时链接预取', 'selectRoute 时预取整条路线的 cloud:// 临时链接并写入缓存；detail 直接命中', '详情页首屏等待更短'],
        ['首页提前预下载', '首页点击难度按钮时提前 loadRoutePackage 并预取路线/图标链接', '跳转动画期间已完成预热'],
        ['图片自动本地降级', '云图片失败后自动切换 /pkg-*/assets 本地图，本地也失败才显示重试', '弱网与云异常时基本无感'],
        ['云端一致性工具', '新增 tools/verify-cloud.js，对比 cloudAssets 与云存储 151 个文件数量/大小', '管理员不再手工核对云端'],
        ['一键发布检查', '新增 tools/release-check.js：生成映射 → 校验 → 冒烟 → 云端检查', '一条命令判断是否可发布'],
        ['隐藏管理台', '新增 pages/admin/admin；长按“我的”版本号进入，提供反馈状态处理与匿名统计汇总', '手机端处理反馈、查看统计'],
        ['管理员绑定码', '管理员身份改为 admin_users 集合 + 一次性绑定码绑定，避免手改 openid 出错', '不再因 l/I 混淆导致无法登录'],
        ['反馈管理增强', '反馈分页加载、截图预览、处理状态记录处理人与处理时间', '管理反馈更完整'],
        ['统计维度增强', '管理台统计增加事件汇总、近14天趋势、热门路线、失败图片 Top', '数据不再只是原始日志'],
        ['图片稳定占位', 'sync-assets.js 生成 data/imageMeta.js 宽高元数据，详情页按比例预留高度', '攻略图加载不再跳动'],
        ['渐进式 JPEG', '路线图与 JPG 图标改为 progressive JPEG 编码', '弱网下先显示模糊全图再清晰'],
    ],
